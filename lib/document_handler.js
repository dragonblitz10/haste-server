...
DocumentHandler.prototype.handlePost = function (request, response) {
  ...
  var onSuccess = function () {
    ...
    if (request.body.length < _this.minLength) {
      cancelled = true;
      winston.warn('document < minLength', { minLength: _this.minLength });
      response.writeHead(400, { 'content-type': 'application/json' });
      response.end(
        JSON.stringify({ message: 'Document is less then ' + _this.minLength + ' characters.' })
      );
      return;
    }
